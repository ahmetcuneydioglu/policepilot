/**
 * AiChat — SigortaOS AI sohbet gövdesi (paylaşılan).
 * İki yerde yaşar: (tabs)/ai tam ekranı ve /ai-sheet yarım-ekran sheet'i.
 * Mesaj durumu bileşenin içindedir; "Temizle" chip'i sohbet doluyken üstte görünür.
 */
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? keyboardOffset : 0}>
      ...
      >
        {empty ? (
          ...
          <>
            <TouchableOpacity style={styles.clearChip} onPress={() => setMessages([])} activeOpacity={0.7}>
              <Text style={styles.clearChipText}>Sohbeti temizle</Text>
            </TouchableOpacity>
            {messages.map((m, i) => (
      ...
      <View style={[styles.inputBar, { paddingBottom: bottomInset + Spacing.sm }]}>
        <TextInput
          style={styles.input}
          value={input}
        ...
        >
          <Icon symbol="arrow.up" emoji="➤" size={17} color="#fff" weight="bold" />
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  clearChip: { alignSelf: 'center', backgroundColor: Colors.surface, borderRadius: Radius.full, paddingHorizontal: 12, paddingVertical: 6, marginBottom: 10 },
  clearChipText: { ...Type.caption, color: Colors.secondary, fontWeight: '700' },
  ...
  inputBar: { flexDirection: 'row', alignItems: 'flex-end', gap: 8, paddingHorizontal: Spacing.md, paddingTop: Spacing.sm, borderTopWidth: 1, borderTopColor: Colors.border, backgroundColor: Colors.card },
